classdef cobss < handle
    %COBSS Consistent Overhead Byte Stuffing Streamer
            ...
            switch data
                ...
                otherwise
                    ...
                    if obj.count > 255 % max stuffing code value - indicate 254 characters followed by the next code
                        ...
                        obj.count = 1;
                        obj.index = length(obj.data);
                    end
            end
        end
        
        function data = get(obj)
            if obj.count == 1; % if last data was a 0
                obj.data(end+1) = 1; % add a phantom '0' so that the decoder doesn't remove a real one
            else
                obj.data(obj.index) = obj.count; % complete last block
            end
            data = obj.data;
        end
    end  
